...
export async function GET() {
  try {
    ...
    for (const file of pdfFiles) {
      newContent += <li><a href=/pdfs/standards/ target=_blank rel=noopener noreferrer></a></li>;
    }
    newContent += </ul><hr/><br/>;
    ...
    let kmAdded = 0;
    for (const file of pdfFiles) {
      const exists = await prisma.document.findFirst({ where: { url: /pdfs/standards/ }});
      if (!exists) {
        await prisma.document.create({
          data: {
            title: file.replace('.pdf', ''), type: 'PDF',
            url: /pdfs/standards/, category: 'มาตรฐานแหล่งท่องเที่ยว'
          }
        });
        kmAdded++;
      }
